...
public static class Seed
{
    public static async Task SeedData(this RadzenBookDbContext context, UserManager<AppUser> userManager, RoleManager<AppRole> roleManager)
    {
        ...
        for (var i = 1; i <= 3; i++)
        {
            var employeeUser = new AppUser { UserName = $"employee{i}@example.com", Email = $"employee{i}@example.com" };
        ...
        for (var i = 1; i <= 5; i++)
        {
            var customerUser = new AppUser { UserName = $"customer{i}@example.com", Email = $"customer{i}@example.com" };
